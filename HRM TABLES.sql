...
REM========================EMP_SALARY_SECTOR========================
DROP TABLE SALARY_SECTORS;

CREATE TABLE SALARY_SECTORS
(
  ID               NUMBER GENERATED ALWAYS AS IDENTITY(START WITH 1),  
  ...
  MODIFIED_ON TIMESTAMP, 
  MODIFIED_TIME VARCHAR2(15), 
  CONSTRAINT SALARY_SECTORS_PK PRIMARY KEY(ID)  
);
/
CREATE OR REPLACE TRIGGER SALARY_SECTORS_TGR
BEFORE INSERT OR UPDATE ON SALARY_SECTORS
FOR EACH ROW 
BEGIN
IF INSERTING THEN
    :NEW.COMPANY_CODE := APEX_UTIL.GET_SESSION_STATE('GLOBAL_COMPANY_CODE');
